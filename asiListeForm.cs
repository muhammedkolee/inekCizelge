...
{
    public partial class asiListeForm : Form
    {
        ...
        private void asiListeForm_Load(object sender, EventArgs e)
        {
            sqlBaglanti();
            veriYukle();
            butonDuzen();
            asiSilButon.Width = 75;
        }

        private void asiListeForm_Resize(object sender, EventArgs e)
        {
            butonDuzen();
        }
        ...
        private void anaDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (e.ColumnIndex == anaDataGridView.Columns["asiSilButon"].Index)
                {


                    DialogResult result = MessageBox.Show(
                        $"Kayıt silinecek:\nKüpe No: {anaDataGridView.Rows[e.RowIndex].Cells["kupeNo"].Value.ToString()}\nAşı İsim: {anaDataGridView.Rows[e.RowIndex].Cells["asiAdi"].Value.ToString()}\nEmin misiniz?",
                        "Silme Onayı",
                    ...
                    {
                        string query = "delete from asiListesi where kupeNo = @kupeNo";

                        using (SQLiteCommand command = new SQLiteCommand(query, conn))
                        {
                            command.Parameters.AddWithValue("@kupeNo", anaDataGridView.Rows[e.RowIndex].Cells["kupeNo"].Value.ToString());
        ...
        {
            string query = "select * from asiListesi";
            ...
            using (SQLiteCommand command = new SQLiteCommand(query, conn))
            {
                using (SQLiteDataAdapter dadapter = new SQLiteDataAdapter(command))
                {
                    ...
                    anaDataGridView.Columns["asiAdi"].HeaderText = "Aşı Adı";
                    anaDataGridView.Columns["asiTarihi"].HeaderText = "Aşı Tarihi";
